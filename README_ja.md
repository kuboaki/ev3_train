...
2. 起動すると、警告音が鳴り、走行指示待ちになります。
3. 列車を線路の上に置き、車輪をレールに載せます。
3. 運転ボタンを押すと、確認音が鳴り、走行します。
4. 信号読取り部（上向きに装備したカラーセンサー）が、停止信号（赤）を読み取ると、到着音が鳴り、停止します。
5. 信号読取り部が、進行信号（緑）を読み取ると、確認音が鳴り、走行を再開します。
4. 運転ボタンを押すと、運転終了状態で停止します。

### 信号機を操作する

1. 転送したプログラムを起動します。
2. 起動すると、確認音が鳴り、モーターが回転して準備動作に入ります。
3. 停止信号（赤が下向き）を現示すると、モーターが停止します。
4. 列車が信号まで来ると、停止信号を読み取って停車します。
5. 手動スイッチを押すと、モーターが回転して、信号機が進行信号（線路側が緑）を現示します。
